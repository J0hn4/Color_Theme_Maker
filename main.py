#Color Theme Maker
import tkinter
from tkinter import BOTH, TRUE, IntVar, DISABLED, NORMAL, filedialog


#Define window
root = tkinter.Tk()
root.title('Color Theme Maker')
photo = tkinter.PhotoImage(file="003-colour.png")
...
def get_red(slider_value):
    """Turn current slider value for red into a hex value and update color. The scale vallue is passed automatically
    when the scale is moved calling the get_red function"""
    global red_value

    #Turn slider value into an int and hex value. Strip leading chars so only two remain
    red_value = hex(int(slider_value))
    red_value = red_value.lstrip("0x")

    #If hex value is single digit, lead with a 0 such that f becomes 0f
...
def update_color():
    """Update the current color box based on teh lisder values. Display tuple and ehx values of the current color."""
...
def store_color():
    ...
    #Get the current value of each slider and append 0's to keep formattin
    ...
    new_color_tuple = tkinter.Label(output_frame, width=14, text='(' + red + '),(' + green + '),(' + blue + ')')
    ...
    new_color_black_box.grid(row=stored_color.get(), column=4, padx=20, ipady=5)
    new_color_box.grid(row=stored_color.get(), column=4)

    #Move the readio button stored colors_ to the next vlue if available
    if stored_color.get() < 5:
        stored_color.set(stored_color.get() + 1)

def save_colors():
    """Output the chosen colors to a txt file"""
    #get the directory where the used would like to save it
    ...
    with open(file_name, "w") as f:
        f.write("Color Them Maker Output \n\n")
...
#Create the labels, sliders, and buttons for each color RBG
...
#Put labels, slidrs, and buttons on to the frame... Use ipadx with rgb buttons to define column width
red_label.grid(row=0, column=0, sticky="W")
red_slider.grid(row=1, column=0, sticky="W")
red_button.grid(row=2, column=0, sticky="EW", pady=1, padx=1, ipadx=20)

green_label.grid(row=0, column=1, sticky="W")
green_slider.grid(row=1, column=1, sticky="W")
green_button.grid(row=2, column=1, sticky="EW", pady=1, padx=1, ipadx=15)

blue_label.grid(row=0, column=2, sticky="W")
blue_slider.grid(row=1, column=2, sticky="W")
blue_button.grid(row=2, column=2, sticky="EW", pady=1, padx=1, ipadx=18)

yellow_button.grid(row=3, column=0, sticky="EW")
cyan_button.grid(row=3, column=1, sticky="EW")
magenta_button.grid(row=3, column=2, sticky="EW")

store_button.grid(row=4, column=0, columnspan=3, sticky="EW")

save_button.grid(row=4, column=3, sticky="EW")
quit_button.grid(row=4, column=4, sticky="EW")
...
#Create a dictionary to hold al lstored colors
stored_colors = {}
stored_color = IntVar()

#Create Radio buttons t oselect stored colros and populate each row with placeholder values
for i in range(6):
    radio = tkinter.Radiobutton(output_frame, variable=stored_color, value=i)
    radio.grid(row=i, column=0, stick="W")
    ...
    recall_button.grid(row=i, column=1, padx=20, ipadx=5, ipady=5)
    new_color_tuple.grid(row=i, column=2, padx=20, ipadx=5, ipady=5)
    new_color_hex.grid(row=i, column=3, padx=20, ipadx=5, ipady=5)
    new_color_black_box.grid(row=i, column=4, padx=20, ipadx=5, ipady=5)
    new_color_box.grid(row=i, column=4)

    #.cget() returns the value of a specific option. Store the text value of the tuple lable and hex label
    stored_colors[stored_color.get()] = [new_color_tuple.cget('text'), new_color_hex.cget("text")]
